...
bool graphics_capture_is_supported()
{
    try {
...
int screenshot_window_once(void* windowHandle, unsigned char** imageData, int* dataSize, int* width, int* height)
{
    try {
...
int screenshot_window_loop_start(void* windowHandle, int intervalMs)
{
    try {
        if (!g_isInitialized || !g_screenEngine) {
            Logger::Error("Native library not initialized");
...
int screenshot_window_loop_read(unsigned char** imageData, int* dataSize, int* width, int* height)
{
    try {
        if (!g_isInitialized || !g_screenEngine) {
            Logger::Error("Native library not initialized");
...
int screenshot_window_loop_stop()
{
    try {
        if (!g_isInitialized || !g_screenEngine) {
            return static_cast<int>(ResultCode::Success);
...
void free_image_data(unsigned char* imageData)
{
    if (imageData) {
        delete[] imageData;
    }
...
int ocr_image_data(const unsigned char* imageData, int dataSize, int width, int height, OcrResult** result)
{
    try {
...
int ocr_screen_region(int x, int y, int width, int height, OcrResult** result)
{
    try {
        if (!g_isInitialized || !g_ocrEngine) {
            Logger::Error("Native library not initialized");
...
int find_window_by_title(const char* windowTitle, void** windowHandle)
{
    try {
...
int find_window_by_class(const char* className, void** windowHandle)
{
    try {
...
int get_window_rect(void* windowHandle, int* left, int* top, int* right, int* bottom)
{
    try {
...
int is_window_visible(void* windowHandle, bool* isVisible)
{
    try {
...
int get_system_info(const char** osVersion, const char** architecture)
{
    try {
